...
def check_and_uppdate(num):
    ...
    if num<count:  #如果要求可用数目小于可更新数
        ...
        shuffled_indices = np.random.permutation(count) #生成一个随机序列
        print('共可更新：{}条,可用数：{}条,本次更新：{}条'.format(count,available,update))
        k=1
        for i in range(update):
            print(status_errors[shuffled_indices[i]]['status'], ' ', status_errors[shuffled_indices[i]]['inuse'], '\n',k, '/', update)
            password = database.get_user_passord(status_errors[shuffled_indices[i]]['username'])
            updatesession_one(status_errors[shuffled_indices[i]]['username'], password)
            k+=1
